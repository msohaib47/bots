...
Ported from DayTradingBot/position_manager.py (v1), logic unchanged, with one
deliberate difference: v1 called `common.notifier.notify()` directly inline on
every close/scale-out. Per PLAN.md's centralized-notification design, this
module instead appends `trade_close`/`trade_partial_close` events to the
shared kv_store event log -- notifier_service.py is now the only place that
calls notify(), anywhere in the system.

trades.csv columns: timestamp/action/symbol/underlying/type/contracts/price/
pnl/reason, plus EXTRA_COLUMNS below (strike/expiration/underlying_price/
premium_pct/RSI/ADX/ATR/EMA9/EMA21/VWAP/htf_ema21/htf_slope/ema_gap_atr on
OPEN rows, hold_minutes on CLOSE/PARTIAL_CLOSE) -- same shape as v1 and as
backtest.py, so results can be analyzed the same way across all three.
"""
import json
import os
import logging
...
# Injectable so backtest/run_backtest.py can point diagnostic underlying-price
# lookups (see _underlying_price below) at a SimulatedBroker instead of the
# real Alpaca client -- defaults to the real `alpaca` module, unchanged live
# behavior. Not used for anything that gates a trading decision, only for a
# best-effort CSV diagnostic column.
_broker = alpaca
...
def _now_iso():
    return sim_clock.now_utc().isoformat()
...
def register_open(state: dict, contract: dict, qty: int, filled_price: float, order_id: str,
                   sig: dict | None = None):
    """
    `sig` is the signal payload that produced this entry (from `signal.<SYM>`,
    execution_service.py has it in hand already) -- recorded on the OPEN row
    so trades.csv carries the same entry diagnostics backtest.py does.
    Does NOT append a `trade_open` event itself -- execution_service.py does
    that right after calling this, since it also needs the account name.
    """
    sym = contract['symbol']
    state[sym] = {
...
def _hold_minutes(opened_at: str) -> float:
    return round((sim_clock.now_utc() - datetime.fromisoformat(opened_at)).total_seconds() / 60, 1)
...
def _underlying_price(underlying: str):
    ...
    try:
        return _broker.get_latest_price(underlying)
    except Exception:
        return ''
...
def set_cooldown(cooldowns: dict, underlying: str, direction: str):
    ...
    expires = sim_clock.now_utc() + timedelta(minutes=COOLDOWN_MINUTES)
...
def update_cooldown_reset(cooldowns: dict, underlying: str, price: float | None,
    ...
    """
    Call once per tick with the underlying's current indicators (from the
    execution service's in-memory mirror of the `signal.<SYM>` stream) while
    it's in cool-down. Marks the cool-down's signal as reset the first time
    the stopped-out direction's setup actually breaks down.
    """
...
def is_in_cooldown(cooldowns: dict, underlying: str) -> bool:
    ...
    if c.get('reset_seen'):
        return False
    return sim_clock.now_utc() < datetime.fromisoformat(c['until'])


# ── Daily realized P&L (per symbol + total, resets each calendar day) ──────────

def _today_str() -> str:
    return sim_clock.now_utc().date().isoformat()
...
def _emit_event(publisher, event_type: str, payload: dict):
    ...
    Both paths matter: the kv_store event log is what the Notifier catches up
    on after being down; the ZMQ push is what gets it there instantly while
    it's already running. One call site keeps them from drifting apart.
    """
...
def check_and_update_stops(state: dict, current_prices: dict, cooldowns: dict, daily: dict,
                            account_name: str, publisher=None) -> tuple:
    """
    ...
    and appends a `trade_close`/`trade_partial_close` event to the shared event
    log for each -- notifier_service.py turns these into the actual ntfy push.
    ...
    for sym, pos in state.items():
        ...
        if HALF_CLOSE_ENABLED and not pos.get('half_closed') and pct_gain >= HALF_CLOSE_PROFIT_PCT:
            ...
            if half_qty >= 1:
                ...
                log_trade('PARTIAL_CLOSE', sym, pos['underlying'], pos['type'],
                          half_qty, current, partial_pnl, reason=reason,
                ...
                _emit_event(publisher, 'trade_partial_close', {
                    'account': account_name, 'symbol': sym, 'underlying': pos['underlying'],
                    'type': pos['type'], 'contracts': half_qty, 'price': current,
                    'pnl': round(partial_pnl, 2), 'reason': reason,
                })
        ...
        if current <= pos['stop_price']:
            ...
            _emit_event(publisher, 'trade_close', {
                'account': account_name, 'symbol': sym, 'underlying': pos['underlying'],
                'type': pos['type'], 'contracts': pos['contracts'], 'price': current,
