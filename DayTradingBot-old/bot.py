"""
0DTE Day Trading Bot — SPY, QQQ, IWM options
Runs every minute. Strategy: VWAP + EMA crossover + RSI.
Max 2 contracts per position. Stop-loss 30%. Trailing stop from +50%.
...
Usage:
  ...
  python bot.py --backtest 3 -- backtest 3 months (1-12); optional: [symbol ...]
"""
import logging
import os
import sys
...
if __name__ == '__main__':
    ...
    if '--status' in args:
        ...
        if idx + 1 < len(args) and args[idx + 1].isdigit():
            ...
            if not 1 <= months <= 12:
                print('--backtest months must be 1–12')
                sys.exit(1)
