...
from signals import (_ema, _rsi, _vwap, _recent_cross,
                      HTF_SYMBOL, EMA_CROSS_LOOKBACK, VOL_AVG_PERIOD,
...
# -- HTF (SPY 15-min trend) — mirrors signals._htf_trend_bullish() --------------

def _build_htf_series(spy_bars: list) -> tuple:
    """
    Aggregate SPY's 5-min market-hours bars into 15-min bars (day-local chunks
    of 3, matching how Alpaca's 15Min timeframe aligns to the 9:30 open) and
    return (sorted timestamps, sorted closes) for lookup.
    """
    by_day = _group_by_day(spy_bars)
    times, closes = [], []
...
def _htf_lookup_factory(spy_bars: list):
    """Returns a function: cutoff timestamp -> bool|None (SPY 15-min trend as of that time)."""
    import bisect
    times, closes = _build_htf_series(spy_bars)
...
def backtest_symbol(symbol: str, months: int, htf_lookup) -> list:
...
def run_backtest(months: int, symbols: list = None):
    ...
    sys.stdout.write(f'  {HTF_SYMBOL:<6}  (HTF trend reference) ')
    sys.stdout.flush()
    spy_bars, spy_new = _fetch(HTF_SYMBOL, months)
    print(f'{len(spy_bars):>5} bars  ({"+" + str(spy_new) + " new" if spy_new else "cached"})')
    htf_lookup = _htf_lookup_factory(spy_bars)

    all_trades: list = []
    for sym in run_syms:
        all_trades.extend(backtest_symbol(sym, months, htf_lookup))
