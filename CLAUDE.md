...
@.memory/project_daytradingbot_strategy.md

## Deployment

All bots run on Ubuntu server `192.168.1.250` (user: `claude`). The `Z:\work\bots` directory is the Windows source — it is mounted on the server at `/mnt/asus/work/bots` and symlinked as `~/bots`. Edits here are live immediately on the server.
...
- Signal: EMA9/EMA21 golden/death cross + RSI filter (buy if RSI < 65, sell if RSI > 72)
- Uses daily bars (`get_daily_bars`) for indicators; fractional notional orders
...
- Only executes stock types (`ST`, `stock`, `''`); options/other types are logged as skipped
