...
from config import (SYMBOLS, MAX_CONTRACTS_PER_SYMBOL, STATE_FILE, LOG_FILE,
...
def run():
    ...
    for symbol in SYMBOLS:
        ...
        # concurrent position on the same underlying (see MAX_CONTRACTS_PER_SYMBOL
        # in config.py: sizing controls how many contracts that one position holds,
        # not how many separate positions can exist).
        if pm.count_positions_for(state, symbol) >= 1:
            logger.info(f'{symbol}: already has an open position, skipping signal check')
            continue
        ...
        # open-exposure limit (total premium tied up across all open positions
        # <= MAX_OPEN_EXPOSURE x (1 + tolerance)).
        ...
        notify('BUY', contract['symbol'], f'${filled_price:.2f}', f'{symbol} {opt_type.upper()} x{qty} | stop=${round(filled_price*(1-STOP_LOSS_PCT),2)}', bot='DT-Bot-200')
